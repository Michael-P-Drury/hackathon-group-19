...
async def render_map(destination, persona):
    ...
    # Delete any files beyond the 5 most recent
    for old_file in saved_maps[5:]:
        try:
            os.remove(old_file)
        except Exception as e:
            print(f"Could not delete old map {old_file}: {e}")
    ...
    filename = f"map_output.html" # Fixed name for speed
    filepath = os.path.join(assets_dir, filename)
